...
const AdminTeacherEdit = () => {
  ...
  const fetchTeacher = async () => {
    setLoading(true);
    try{
      const response = await axios.get(`${baseUrl}Teacher/${id}`, { // thực hiện một yêu cầu HTTP GET tới server de lay id teacher
        headers: {
  ...
  const fetchDepartments = async()=>{
    try{
      const response = await axios.get(baseUrl + "Department/GetAll", { //endpoint lay all khoa
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      setDepartments(response.data);
    } catch(error){          //xu ly loi khi goi api
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        name="birthDate"
        type="date"
        value={teacher.birthDate.format("YYYY-MM-DD")}
        onChange={(e) =>
          setTeacher((prevTeacher) => ({
            ...prevTeacher,
